...
public class DaoBuildProvider extends AbstractDaoProvider implements BuildProvider {
  ...
  private int createBuildRecord(NewBuild newBuild, int projectId) {
    ...
    return jdbc.query(sql, new SqlParamsBuilder()
        ...
        .withBoolean("aet_update_url_blank", config.isUpdateUrlBlankAfterEachTest())
        .withBoolean("aet_reset_timeouts", config.isResetTimeoutsAfterEachTest())
        .withBoolean("aet_delete_all_cookies", config.isDeleteAllCookiesAfterEachTest())
